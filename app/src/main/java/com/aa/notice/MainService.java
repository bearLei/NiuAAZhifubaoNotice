...
public class MainService extends Service implements Runnable, MediaPlayer.OnCompletionListener {
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            mNotificationChannel = new NotificationChannel(CustomApplcation.CHANNEL_Front, "pxapy", NotificationManager.IMPORTANCE_DEFAULT);
            mNotificationChannel.setDescription("支付的监控");
            mNM.createNotificationChannel(mNotificationChannel);
        }
        NotificationCompat.Builder nb = new NotificationCompat.Builder(this,CustomApplcation.CHANNEL_Front);//
        nb.setContentTitle("支付通知").setTicker("支付监控").setSmallIcon(R.drawable.ic_logo);
